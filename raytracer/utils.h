...
inline void matRayMult(double A[4][4], struct ray3D *ray){
    //result is left in *ray
    matVecMult(A,&ray->p0);
    matVecMult(A,&ray->d);
}
...
/*
inline void copyRay(struct ray3D *source,struct ray3D *dest){
    if(source==NULL) return;
    if(dest==NULL) return;
    dest->p0=source->p0;
    dest->d=source->d;
    dest->rayPos=source->rayPos;
}
*/

inline void copyPoint(struct point3D *src,struct point3D *dest){
    if(src==NULL) return;
    if(dest==NULL) return;
    dest->px=src->px;
    dest->py=src->py;
